...
?>
    ...
    <fieldset>
        ...
        if (testPost($Min))
        {
             $req = $bdd->prepare("SELECT * FROM annonce WHERE nb de chambre <=? ORDER BY nb de chambre DESC");
             $req->execute(array($_GET['nb de chambre']));
    
    while ($donnees = $req->fetch())
        {
           echo $donnees['nb de chambre']; 
        }
...
<?php
         ...
         if (testPost($Min))
        {
             $req = $bdd->prepare("SELECT * FROM annonce WHERE superficie <=? ORDER BY superfie DESC");
             $req->execute(array($_GET['superficie']));
    
    while ($donnees = $req->fetch())
        {
           echo $donnees['superficie']; 
        }
        ...
        if (testPost($Max))
        {
             $req = $bdd->prepare("SELECT * FROM annonce WHERE superficie =>? ORDER BY nb de chambre DESC");
             $req->execute(array($_GET['superficie']));
